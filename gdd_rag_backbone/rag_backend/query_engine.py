...
async def ask_question(
    doc_id: str, query: str, *, mode: str = "mix", debug: bool = False, **query_kwargs
) -> str:
    """
    ...
    if hasattr(rag, "aquery"):
        response = await rag.aquery(query=query, mode=mode, **query_kwargs)
    else:
        # Fallback to sync query if aquery not available
        response = rag.query(query=query, mode=mode, **query_kwargs)

    return response


async def debug_query(doc_id: str, query: str, *, mode: str = "mix", **query_kwargs) -> None:
    """
    ...
    QueryParam(query=query, mode=mode, **query_kwargs)
