...
class EvaluationResult:
    ...
    retrieval_rank: Optional[
        int
    ]  # Rank of the correct chunk in retrieved results (1-based, None if not found)
...
Question:"""
        ...
        try:
            ...
            first_sentence = (
                chunk_content.split(".")[0] if "." in chunk_content else chunk_content[:100]
            )
    ...
    def validate_question(self, question: str) -> Dict[str, any]:
        ...
        has_structure = "?" in question or any(
            q in question.lower()
            for q in ["what", "how", "why", "when", "where", "which", "who", "describe", "explain"]
        )
    ...
    def generate_test_cases(
        ...
        if one_per_doc:
            ...
            if len(all_chunks) < num_cases:
                print(
                    f"Warning: Only {len(all_chunks)} chunks available, generating {len(all_chunks)} test cases"
                )
                num_cases = len(all_chunks)
    ...
    def evaluate_single(
        ...
        try:
            retrieved_chunks, generated_answer = retrieval_func(
                test_case.generated_question, [test_case.doc_id]
            )
        except Exception as e:
    ...
    def evaluate_batch(
        ...
        for i, test_case in enumerate(test_cases):
            print(
                f"Evaluating {i + 1}/{len(test_cases)}: {test_case.generated_question[:60]}...",
                end="\r",
            )
    ...
    def _calculate_summary(
        ...
        # Calculate averages
        avg_answer_sim = (
            sum(answer_similarities) / len(answer_similarities) if answer_similarities else 0.0
        )
        avg_top_chunk_sim = (
            sum(top_chunk_similarities) / len(top_chunk_similarities)
            if top_chunk_similarities
            else 0.0
        )
        avg_retrieval_rank = (
            sum(retrieval_ranks) / len(retrieval_ranks) if retrieval_ranks else None
        )
        ...
        retrieval_success = sum(
            1 for r in valid_results if r.retrieval_rank is not None and r.retrieval_rank <= 5
        )
        ...
        high_similarity_rate = (
            high_similarity / len(answer_similarities) if answer_similarities else 0.0
        )
    ...
    def save_results(
        ...
        test_cases_path.write_text(
            json.dumps(test_cases_data, indent=2, ensure_ascii=False), encoding="utf-8"
        )
    ...
    def print_summary(self, summary: EvaluationSummary):
        ...
        print(
            f"  Success rate (correct chunk in top 5): {summary.retrieval_success_rate * 100:.1f}%"
        )
        print()
        print("PERFORMANCE:")
