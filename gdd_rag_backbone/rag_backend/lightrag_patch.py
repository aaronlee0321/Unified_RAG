...
def patch_lightrag():
    ...
    try:
        ...
        try:
            ...
            print(
                "[PATCH] Warning: lightrag not installed. RAGAnything features will not be available."
            )
            print(
                "[PATCH] To use RAGAnything, upgrade to Python 3.10+ and install: pip install lightrag lightrag-hku"
            )
            return
        ...
        try:
            ...
            if not hasattr(utils_module, "get_env_value"):
                ...
                try:
                    ...
                    utils_py_path = os.path.join(
                        os.path.dirname(lightrag.__file__), "utils.py")
                    if os.path.exists(utils_py_path):
                        spec = importlib.util.spec_from_file_location(
                            "lightrag.utils_old", utils_py_path
                        )
                        ...
                        if hasattr(old_utils, "get_env_value"):
                            ...
                            print(
                                "[PATCH] Patched get_env_value into lightrag.utils")
                    else:
        ...
        try:
            if "lightrag.mineru_parser" not in sys.modules:
                ...
                mineru_parser_module = types.ModuleType(
                            ...
                            for file in files:
                                if file.endswith(".py") and not file.startswith("__"):
                                    rel_path = os.path.relpath(
                                        os.path.join(root, file), lightrag_path
                                    )
                                    module_path = "lightrag." + rel_path.replace(
                                        os.sep, "."
                                    ).replace(".py", "")
                                    try:
                                        ...
                                        if hasattr(module, "LightRAG"):
                                            lightrag.LightRAG = module.LightRAG
                                            print(
                                                f"[PATCH] Patched LightRAG from {module_path}")
                                            break
