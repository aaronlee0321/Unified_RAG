...
def insert_alias(keyword: str, alias: str, language: str = "en") -> Dict[str, Any]:
    ...
    result = (
        ...
        .insert({"keyword": keyword.strip(), "alias": alias.strip(), "language": lang_normalized})
        .execute()
    )
...
def find_keyword_by_alias(alias: str) -> List[Dict[str, Any]]:
    ...
    if result.data:
        for row in result.data:
            keywords.append(
                {"keyword": row["keyword"], "alias": row["alias"], "language": row["language"]}
            )
...
def list_all_aliases(
    filter_language: Optional[str] = None, search_term: Optional[str] = None
) -> List[Dict[str, Any]]:
    """
...
def delete_alias(keyword: str, alias: str) -> bool:
    ...
    try:
        ...
        for row in all_aliases:
            if (
                row["keyword"].lower() == keyword_clean.lower()
                and row["alias"].lower() == alias_clean.lower()
            ):
                ...
                result = (
                    client.table("keyword_aliases")
                    .delete()
                    .eq("keyword", exact_keyword)
                    .eq("alias", exact_alias)
                    .execute()
                )
                return len(result.data) > 0 if result.data else False
...
def update_document_metadata(
    ...
    try:
        ...
        result = (
            client.table("keyword_documents").update(update_data).eq("doc_id", doc_id).execute()
        )
...
def list_aliases_grouped() -> Dict[str, Dict[str, Any]]:
    ...
    for row in result.data:
        ...
        grouped[keyword]["aliases"].append(
            {"id": str(row["id"]), "name": row["alias"], "createdAt": row["created_at"]}
        )
