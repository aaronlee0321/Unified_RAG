...
def keyword_search(
    keyword: str, limit: int = 100, doc_id_filter: Optional[str] = None
) -> List[Dict[str, Any]]:
    """
    ...
    logger.info("=" * 80)
    logger.info("[KEYWORD SEARCH] Function called")
    logger.info(
        f"[KEYWORD SEARCH] keyword: '{keyword}' (type: {type(keyword)}, length: {len(keyword) if keyword else 0})"
    )
    ...
    try:
        ...
        try:
            ...
            result = client.rpc("keyword_search_documents", rpc_params).execute()
            ...
            logger.error(
                f"[KEYWORD SEARCH] RPC error args: {rpc_error.args if hasattr(rpc_error, 'args') else 'N/A'}"
            )
            import traceback
        ...
        logger.info(
            f"[KEYWORD SEARCH] Result.data type: {type(result.data) if hasattr(result, 'data') else 'No data attr'}"
        )
        logger.info(
            f"[KEYWORD SEARCH] Result.data is None: {result.data is None if hasattr(result, 'data') else 'N/A'}"
        )
        ...
        if hasattr(result, "data"):
            ...
            if data_length > 0:
                logger.info(
                    f"[KEYWORD SEARCH] First result keys: {list(result.data[0].keys()) if isinstance(result.data[0], dict) else 'Not a dict'}"
                )
                ...
                for idx, item in enumerate(result.data[:5]):
                    ...
                    logger.info(
                        f"  [{idx + 1}] chunk_id={chunk_id}, relevance={relevance}, doc_id={doc_id}, section={section}, content_preview='{content_preview}...'"
                    )

        final_result = result.data if result.data else []
        logger.info(
            f"[KEYWORD SEARCH] Returning {len(final_result)} results for keyword '{keyword_stripped}'"
        )

        # Log relevance score distribution
        if final_result:
            relevances = [
                r.get("relevance", 0.0) for r in final_result if r.get("relevance") is not None
            ]
            if relevances:
                logger.info(
                    f"[KEYWORD SEARCH] Relevance score stats: min={min(relevances):.4f}, max={max(relevances):.4f}, avg={sum(relevances) / len(relevances):.4f}, count>0={sum(1 for r in relevances if r > 0.0)}"
                )
            else:
                logger.warning("[KEYWORD SEARCH] No relevance scores found in results!")
