...
def parse_section_targets(query: str) -> Tuple[str, Dict]:
    ...
    for match in matches:
        ...
        is_doc_id = (
            "[" in match  # Brackets indicate doc_id like [Asset,_UI]
            or match.count("_") >= 2  # Multiple underscores indicate doc_id
            or "("
            in match  # Parentheses indicate doc_id like Asset_UI_Tank_War_Tank_Selection_Screen_Design_(Cơ_chế_chọn_tank)
            or ".md" in match.lower()  # File extension
        )
