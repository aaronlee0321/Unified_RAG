...
from backend.upload_jobs import get_job, new_job, update_job
...
def run_code_upload_pipeline_async(job_id, file_bytes, filename):
    ...
    try:
        ...
        if not filename.lower().endswith(".cs"):
            update_job(
                job_id, status="error", step="Failed", message="Only .cs files are supported"
            )
            return

        update_job(job_id, step="Extracting methods and classes")

        from backend.code_service import build_code_upload_chunks
        ...
        method_chunks, class_chunks = build_code_upload_chunks(
            code_text, file_path)
...
# Log PORT environment variable (optional override for deployment)
...
try:
    from backend.code_service import list_indexed_files, query_codebase
...
def gdd_upload():
    ...
    try:
        ...
        t = threading.Thread(
            target=run_upload_pipeline_async, args=(
...
def gdd_documents():
    ...
    try:
        ...
        app.logger.info("=" * 60)
        return jsonify({"documents": all_documents, "options": options})
    except Exception as e:
...
def get_gdd_sections():
    ...
    try:
        ...
        result = {"sections": sections, "doc_id": doc_id}
        app.logger.info(
...
def explainer_search_stream():
    ...
    try:
        ...
        def error_stream():
            yield f"data: {json.dumps({'message': f'❌ Error: {err_msg}'})}\n\n"
...
def explainer_get_pdf_url():
    ...
    try:
        ...
        if pdf_url:
            ...
            return jsonify({"error": f"No PDF found for document: {doc_id}", "success": False}), 404
    except Exception as e:
...
def explainer_preview():
    ...
    try:
        ...
        try:
            ...
            return jsonify(
                {"error": f"Could not initialize LLM provider: {str(e)}", "success": False}
            ), 500
        ...
        if (
            ...
            or detected_language == "vn"
            or detected_language == "vietnamese"
        ):
            language_instruction = "IMPORTANT: Respond in Vietnamese (Tiếng Việt). Your entire summary must be in Vietnamese."
        else:
            language_instruction = (
                "IMPORTANT: Respond in English. Your entire summary must be in English."
            )
...
def code_upload():
    ...
    try:
        ...
        # Start background thread
        t = threading.Thread(
            target=run_code_upload_pipeline_async,
            args=(job_id, file_bytes, file.filename),
            daemon=True,
        )
        t.start()
...
def debug_code_supabase():
    ...
    try:
        from backend.storage.code_supabase_storage import USE_SUPABASE
        ...
        if USE_SUPABASE:
            try:
                ...
                try:
                    ...
                    client = get_supabase_client()
                    result = (
                        client.table("code_chunks").select(
                            "id", count="exact").limit(1).execute()
                    )
                    diagnostics["code_chunks_count"] = (
                        result.count if hasattr(result, "count") else "unknown"
                    )
                except Exception as e:
...
def debug_supabase():
    ...
    try:
        ...
        if supabase_url and supabase_key:
            try:
                from backend.storage.keyword_storage import list_keyword_documents
                from backend.storage.supabase_client import get_code_files, get_supabase_client

                # Test anon key connection
                get_supabase_client(use_service_key=False)
...
def get_aliases():
    ...
    try:
        ...
        return jsonify({"keywords": keywords_list, "lastUpdated": datetime.now().isoformat()})
    except Exception as e:
        import traceback
...
def save_aliases():
    ...
    try:
        ...
        return jsonify({"status": "success", "lastUpdated": datetime.now().isoformat()})
    except Exception as e:
        import traceback
...
try:
    ...
    traceback.print_exc(file=sys.stderr)
    # Don't raise - let server handle it
